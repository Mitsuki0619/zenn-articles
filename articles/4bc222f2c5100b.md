...
それでも発火するにはするのですが、Next.js の cookie を使うとそのルートは Dynamic Route になってしまうんですよね。ルートの layout.tsx に cookie の処理を含んだコンポーネントを置いてしまうと結果的に全ルートが Dynamic Route になってしまい、Static route や PPR といった選択肢が消えてしまううえ、Cookie を取得し、SSR が完了するまで画面が表示されないため全体的にページ遷移がもっさりして UX が悪くなってしまいます。
